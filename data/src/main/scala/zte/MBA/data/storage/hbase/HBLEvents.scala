...
import org.apache.hadoop.hbase.{HColumnDescriptor, HTableDescriptor, TableName, NamespaceDescriptor}
import org.apache.hadoop.hbase.client.{Delete, Get, HTableInterface, Result}
import org.joda.time.DateTime
import zte.MBA.data.storage.hbase.HBEventsUtil.RowKey
import zte.MBA.data.storage.{Event, LEvents, StorageClientConfig}
...
class HBLEvents(val client: HBClient, config: StorageClientConfig, val namespace: String)
  ...
  def init(appId: Int, channelId: Option[Int] = None): Boolean = {
    ...
    if (!client.admin.tableExists(tableName)) {
      ...
      tableDesc.addFamily(new HColumnDescriptor("r"))
  ...
  def futureFind(
    ...
    Future {
      ...
      val eventsIter: Iterator[Result] = scanner.iterator.asScala
