...
import zte.MBA.controller.{WithPrId, Utils, Params, Engine}
import zte.MBA.controller.Engine.DataSourceMap
import zte.MBA.core.{BaseServing, BaseAlgorithm, Doer}
import zte.MBA.data.storage.{EngineManifest, EngineInstance, Storage}
  ...
  def main(args: Array[String]): Unit = {
    ...
    parser.parse(args, ServerConfig()) map {sc =>
      ...
      engineInstances.get(sc.engineInstanceId) map { engineInstance =>
        ...
        val engineVersion = sc.engineVersion.getOrElse(engineInstance.engineVersion)
...
class MasterActor(
  ...
  def undeploy(ip: String, port: Int): Unit = {
    ...
    try {
      ...
      code match {
        case 200 => Unit
        case 404 => log.error(s"Another process is using $serverUrl. Unable to undeploy.")
        case _ => log.error(
